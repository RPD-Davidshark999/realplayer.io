...
contract LggNft is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    constructor() ERC721("RPDN", "RPD") {
        baseUri = "https://rpd.realplayer.io/upload/meta/";
    }
    ...
    function openBlindBox(uint256 tokenId, string memory uri) external checkPermit(msg.sender) {
        _setTokenURI(tokenId, uri);
    }
    ...
    */
    ...
    function airdrop(address[] memory _tos, string[] memory _uris) public onlyOwner {
        for (uint8 i = 0; i < _tos.length; i++) {
            safeMint(_tos[i], _uris[i]);
        }
    }
